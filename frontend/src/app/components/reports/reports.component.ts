  ...
  template: `
    <div class="fade-in">
      ...
      <div class="card slide-up">
        ...
        >
          <div
            *ngFor="let group of bloodGroupStats"
            style="background: #f8fafc; padding: 1rem; border-radius: 8px; text-align: center;"
          >
            <div
              style="font-size: 1.5rem; font-weight: 700; color: #dc2626; margin-bottom: 0.25rem;"
            >
              {{ group.count }}
            </div>
            <div
              style="font-size: 0.875rem; color: #64748b; margin-bottom: 0.25rem;"
            >
              {{ group.bloodGroup }}
            </div>
      ...
      <div class="card slide-up">
        ...
        >
          <div
            *ngFor="let ageGroup of ageGroupStats"
          ...
          >
            ...
            <div
              style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px; overflow: hidden;"
            >
              <div
      ...
      <div class="card slide-up">
              ...
              <tbody>
                <tr *ngFor="let donation of donations">
                  ...
                  <td>
                    <span
                      [class]="
                        'status-badge status-' + getStatusClass(donation.status)
                      "
                    >
                      {{ donation.status }}
                    </span>
        ...
        <div
          *ngIf="!loadingDonations && donations.length === 0"
          style="text-align: center; padding: 2rem; color: #64748b;"
        >
          No donation records found for
          {{ selectedYear ? selectedYear : "the selected period" }}.
        </div>
      </div>
      ...
      <div class="card slide-up">
        ...
        <div
          style="background: linear-gradient(135deg, #fef3f2 0%, #fee2e2 100%); padding: 2rem; border-radius: 8px;"
        >
          <div
          ...
          >
            <div>
              ...
              <ul style="list-style: none; padding: 0; margin: 0;">
                ...
                <li style="margin-bottom: 0.25rem;">
                  ...
                  <strong>{{ stats?.donated || 0 }}</strong>
                </li>
                <li style="margin-bottom: 0.25rem;">
                  • Success Rate:
                  <strong>{{ stats?.donationRate || 0 }}%</strong>
                </li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul style="list-style: none; padding: 0; margin: 0;">
                <li style="margin-bottom: 0.25rem;">
                  • Total Volume: <strong>{{ getTotalBloodVolume() }}ml</strong>
                </li>
                <li style="margin-bottom: 0.25rem;">
                  • Average per Donor: <strong>450ml</strong>
                </li>
                <li style="margin-bottom: 0.25rem;">
                  • Lives Potentially Saved:
                  <strong>{{ (stats?.donated || 0) * 3 }}</strong>
                </li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul style="list-style: none; padding: 0; margin: 0;">
                <li style="margin-bottom: 0.25rem;">
                  • Health Issues Found:
                  <strong>{{ healthIssuesCount }}</strong>
                </li>
                <li style="margin-bottom: 0.25rem;">
                  • Healthy Donors:
                  <strong>{{ getHealthyDonorsCount() }}</strong>
                </li>
                <li style="margin-bottom: 0.25rem;">
                  • Rejection Rate: <strong>{{ getRejectionRate() }}%</strong>
                </li>
              </ul>
            </div>
          </div>
...
export class ReportsComponent implements OnInit {
  ...
  async loadAvailableYears() {
    try {
      this.availableYears =
        (await this.apiService.getAvailableYears().toPromise()) || [];
    } catch (error) {
  ...
  calculateBloodGroupStats() {
    ...
    usersInPeriod.forEach((user: User) => {
      if (user.bloodGroup) {
        bloodGroupCounts[user.bloodGroup] =
          (bloodGroupCounts[user.bloodGroup] || 0) + 1;
      }
    });

    this.bloodGroupStats = Object.entries(bloodGroupCounts)
      .map(([bloodGroup, count]) => ({
        bloodGroup,
        count,
        percentage:
          usersInPeriod.length > 0
            ? Math.round((count / usersInPeriod.length) * 100)
            : 0,
      }))
  ...
  calculateAgeGroupStats() {
    ...
    this.ageGroupStats = ageGroups
      .map((group) => {
        const count = usersInPeriod.filter(
          (user: User) => user.age >= group.min && user.age <= group.max
        ).length;
  ...
  calculateHealthStats() {
    this.calculateTotalBloodDonated();
    // Get unique users from donations for the selected period
    const uniqueUsers = new Map();
    this.donations.forEach((donation) => {
      if (donation.user && donation.user.id) {
    ...
    const usersInPeriod = Array.from(uniqueUsers.values());
    this.healthIssuesCount = usersInPeriod.filter(
      (user: User) => user.hasHealthIssues
    ).length;
  }
  ...
  exportReport() {
    ...
    const fileName = this.selectedYear
      ...
      : `blood-donation-report-all-time-${
